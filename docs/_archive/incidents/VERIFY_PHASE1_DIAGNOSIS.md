...
- If you want absolute exit-code confirmation, re-run `see runbook Phase-1 gate` without piping and check `$LASTEXITCODE`.

**Exit Code Verification (No Pipeline)**
- Command (PowerShell): `see runbook Phase-1 gate; $LASTEXITCODE`
...
- `see runbook Phase-1 gate`
- If exit code must be captured explicitly in PowerShell: `see runbook Phase-1 gate; $LASTEXITCODE`
