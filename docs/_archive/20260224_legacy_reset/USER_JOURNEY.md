...
## Journey 1 â€” Create a run and inspect results
...
## Journey 2 â€” Reproduce the same run
...
## Journey 3 â€” Error recovery (must be user-explainable)
The UI must show a clear action for each case:
- Runs root missing/misconfigured â†’ show how to configure it
- CSV invalid/unreadable â†’ show what is wrong
- Strategy/indicator validation fails â†’ show which file/field failed
- Run already exists â†’ treat as success or show idempotent message
- Network/API unreachable â†’ show retry guidance
